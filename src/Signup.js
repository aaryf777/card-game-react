...
export default function Signup() {
    ...
    const goNext = async e => {
        let payload = {
          ...
          contact: mobileRef.current.value
        }
  ...
  return (
    submit ? <Otp loginData={signupData}/>
      ...
      <div className='h-80 w-full bg-gray rounded-md m-3 md:w-1/3 lg:w-1/3 flex flex-col items-center gap-8'>
        ...
        <div className='flex flex-col items-center gap-2 w-full'>
          ...
          <div className='py-1 flex gap-1 items-center border-b-2 border-b-gray-2 w-10/12'>
            ...
            <input ref={mobileRef} className='  bg-transparent  outline-none ' type='text' placeholder='Enter Mobile number'/>
          </div>
        ...
        <div className='flex flex-col gap-2 text-center'>
          ...
          <div className='flex text-gray-2 text-sm'>Have an Account ? <Link to='/login' className='text-red font-semibold ml-2 cursor-pointer'> Login</Link></div>
        </div>
